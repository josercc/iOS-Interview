...
```swift
class Solution {
    func insertSort(_ numbers:[Int], _ index:Int = 1, _ max:Int = 0) -> [Int] {
        ...
        let _insertNumber = _numbers[index]
        for item in numbers.enumerated() {
            guard item.offset <= index else {
                break
            }
            if item.element > _insertNumber {
                _numbers.insert(_insertNumber, at: item.offset)
                _numbers.remove(at: index + 1)
                break
            }
        }
        return insertSort(_numbers, (index + 1))
    }
}
```
</details>
